...
After loading ST data, user can run `SecAct.signaling.inference` to infer the activities of 1248 secreted proteins for each spot.

``` r
# infer activity
SpaCET_obj <- SecAct.signaling.inference(SpaCET_obj)
...
WWW <- SpaCET_obj@results$pattern$weight.W

# identify secreted proteins with pattern 3
n <- 1
WWW_n <- WWW[apply(WWW,1,function(x) x[n]==max(x)),]
rownames(WWW_n)
