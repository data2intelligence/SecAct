...
SecAct is an R package designed for inferring the intercellular activity
of secreted proteins from gene expression profiles. Users can input
multiple modalities of expression data, including spatial, single-cell,
or bulk transcriptomics data. The outputs are the inferred activities of
1248 secreted proteins for each ST spot, individual cell, or sample.
...
differentially regulated between two phenotypes.
...
# load the differenrial profile
dataPath <- file.path(system.file(package = "SecAct"), "extdata/")
expr.diff <- read.table(paste0(dataPath, "Ly86.OverExpression_vs_Vector.diff"))

# Run SecAct to infer the activity change
