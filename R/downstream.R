...
SecAct.signaling.pattern.gene <- function(SpaCET_obj, n)
{
  ...
  res <- weight.W[apply(temp,1,function(x) x[n]==max(x)),,drop=FALSE]
  res[order(res[,n],decreasing = TRUE),,drop=FALSE]
}
...
)
{
  ...
  exp <- SpaCET_obj@input$counts
  if(tolower(SpaCET_obj@input$organism)=="mouse") exp <- mouse2human_mat(exp)
